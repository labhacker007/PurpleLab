...
BUILTIN_SCHEMAS: list[LogSourceDefinition] = [
    ...
    LogSourceDefinition(
        ...
        version="1.0",
        format="json",
        description="External attack surface findings: exposed assets, open ports, SSL/TLS issues, subdomain enumeration, and supply chain exposure.",
        fields={
            "asset_id": {"type": "string", "description": "Unique asset identifier"},
            "asset_type": {"type": "string", "description": "domain|ip|certificate|url|service"},
            "asset_value": {"type": "string", "description": "Domain, IP, or URL"},
            "severity": {"type": "string", "description": "critical|high|medium|low|info"},
            "finding_type": {"type": "string", "description": "exposed_service|expired_cert|open_port|subdomain_takeover|leaked_credential"},
            "port": {"type": "integer", "description": "Exposed port number (if applicable)"},
            "service": {"type": "string", "description": "Service name/banner"},
            "first_seen": {"type": "datetime", "description": "First observed timestamp"},
            "last_seen": {"type": "datetime", "description": "Last confirmed active timestamp"},
            "tags": {"type": "array", "description": "Asset tags / labels"},
            "remediation": {"type": "string", "description": "Recommended remediation action"},
        },
        sample_events={
            "exposed_service": [{
                "asset_id": "asm-001", "asset_type": "ip", "asset_value": "203.0.113.42",
                "severity": "high", "finding_type": "exposed_service",
                "port": 22, "service": "OpenSSH 8.9", "first_seen": "2026-03-01T00:00:00Z",
                "last_seen": "2026-06-01T00:00:00Z", "tags": ["prod", "legacy"],
                "remediation": "Restrict SSH to VPN egress IPs via security group",
            }],
            "expired_cert": [{
                "asset_id": "asm-002", "asset_type": "certificate", "asset_value": "api.example.com",
                "severity": "critical", "finding_type": "expired_cert",
                "first_seen": "2026-05-29T00:00:00Z", "last_seen": "2026-06-01T00:00:00Z",
                "tags": ["api", "tls"], "remediation": "Renew TLS certificate immediately",
            }],
        },
        mitre_mappings={
            "T1190": ["exposed_service"],
            "T1133": ["exposed_service"],
            "T1594": ["subdomain_enum"],
        },
        doc_url="",
        doc_last_checked="2026-06-02",
        update_frequency="frequent",
        splunk_sourcetype="asm:finding",
        joti_source_type="asm",
    ),
    ...
    LogSourceDefinition(
        source_id="cmdb",
        vendor="Generic",
        product="CMDB / Asset Inventory",
        category="cmdb",
        version="1.0",
        format="json",
        description="Configuration item (CI) records and change events from CMDB: asset lifecycle, configuration drift, and relationship mapping.",
        fields={
            "ci_id": {"type": "string", "description": "Configuration item identifier"},
            "ci_class": {"type": "string", "description": "Server|Workstation|NetworkDevice|Application|Database|Container"},
            "ci_name": {"type": "string", "description": "Human-readable asset name"},
            "owner": {"type": "string", "description": "Owning team or person"},
            "environment": {"type": "string", "description": "prod|staging|dev|dr"},
            "status": {"type": "string", "description": "active|decommissioned|maintenance|unknown"},
            "ip_addresses": {"type": "array", "description": "Associated IP addresses"},
            "os_version": {"type": "string", "description": "Operating system and version"},
            "patch_level": {"type": "string", "description": "Latest applied patch level"},
            "last_scanned": {"type": "datetime", "description": "Last compliance/vulnerability scan"},
            "change_type": {"type": "string", "description": "created|updated|deleted|drift_detected"},
            "changed_at": {"type": "datetime", "description": "Change event timestamp"},
            "changed_by": {"type": "string", "description": "User or automation that made the change"},
        },
        sample_events={
            "ci_change": [{
                "ci_id": "ci-1234", "ci_class": "Server", "ci_name": "prod-web-01",
                "owner": "platform-eng", "environment": "prod", "status": "active",
                "ip_addresses": ["10.0.1.10"], "os_version": "Ubuntu 22.04 LTS",
                "patch_level": "2026-05-15", "last_scanned": "2026-06-01T02:00:00Z",
                "change_type": "updated", "changed_at": "2026-06-01T14:32:00Z",
                "changed_by": "ansible-automation",
            }],
            "drift_detected": [{
                "ci_id": "ci-5678", "ci_class": "NetworkDevice", "ci_name": "core-switch-01",
                "owner": "netops", "environment": "prod", "status": "active",
                "change_type": "drift_detected", "changed_at": "2026-06-01T09:15:00Z",
                "changed_by": "drift_scanner", "ip_addresses": ["10.0.0.1"],
            }],
        },
        mitre_mappings={
            "T1078": ["ci_change"],
            "T1562": ["drift_detected"],
            "T1195": ["ci_change"],
        },
        doc_url="",
        doc_last_checked="2026-06-02",
        update_frequency="stable",
        splunk_sourcetype="cmdb:ci",
        joti_source_type="cmdb",
    ),
    ...
    LogSourceDefinition(
        ...
        version="1.0",
        format="json",
        description="Cloud misconfiguration and compliance findings: policy violations, public buckets, over-privileged IAM, encryption gaps, and compliance framework deviations.",
        fields={
            "finding_id": {"type": "string", "description": "Unique finding identifier"},
            "cloud_provider": {"type": "string", "description": "AWS|GCP|Azure|OCI"},
            "account_id": {"type": "string", "description": "Cloud account/subscription ID"},
            "region": {"type": "string", "description": "Cloud region"},
            "resource_id": {"type": "string", "description": "ARN, resource ID, or full path"},
            "resource_type": {"type": "string", "description": "S3Bucket|IAMRole|SecurityGroup|RDSInstance|..."},
            "rule_id": {"type": "string", "description": "Policy rule that was violated"},
            "rule_title": {"type": "string", "description": "Human-readable rule description"},
            "severity": {"type": "string", "description": "critical|high|medium|low|info"},
            "status": {"type": "string", "description": "open|remediated|suppressed|in_progress"},
            "compliance_frameworks": {"type": "array", "description": "Relevant frameworks: CIS|SOC2|PCI|HIPAA|NIST"},
            "first_detected": {"type": "datetime", "description": "First detection timestamp"},
            "remediation_guidance": {"type": "string", "description": "Step-by-step fix guidance"},
        },
        sample_events={
            "public_storage": [{
                "finding_id": "cspm-001", "cloud_provider": "AWS", "account_id": "123456789012",
                "region": "us-east-1", "resource_id": "arn:aws:s3:::my-public-bucket",
                "resource_type": "S3Bucket", "rule_id": "S3-001",
                "rule_title": "S3 bucket allows public read access",
                "severity": "critical", "status": "open",
                "compliance_frameworks": ["CIS", "SOC2", "PCI"],
                "first_detected": "2026-05-28T10:00:00Z",
                "remediation_guidance": "Remove public bucket policy and enable Block Public Access settings",
            }],
            "iam_violation": [{
                "finding_id": "cspm-002", "cloud_provider": "AWS", "account_id": "123456789012",
                "region": "global", "resource_id": "arn:aws:iam::123456789012:role/AdminRole",
                "resource_type": "IAMRole", "rule_id": "IAM-007",
                "rule_title": "IAM role with administrator access attached to EC2 instance",
                "severity": "high", "status": "open",
                "compliance_frameworks": ["CIS", "NIST"],
                "first_detected": "2026-06-01T06:00:00Z",
                "remediation_guidance": "Replace AdministratorAccess with least-privilege policy",
            }],
        },
        mitre_mappings={
            "T1530": ["public_storage"],
            "T1078.004": ["iam_violation"],
            "T1552.005": ["secret_leak"],
        },
        doc_url="",
        doc_last_checked="2026-06-02",
        update_frequency="frequent",
        splunk_sourcetype="cspm:finding",
        joti_source_type="cspm",
    ),

    # ── Product / Service Registry ────────────────────────────────────────────
    LogSourceDefinition(
        source_id="product_registry",
        vendor="Internal",
        product="Product & Service Registry",
        category="product_registry",
        version="1.0",
        format="json",
        description="Internal product and microservice inventory: service metadata, dependency graph, SLA tiers, data classification, and ownership records.",
        fields={
            "service_id": {"type": "string", "description": "Unique service identifier (slug)"},
            "service_name": {"type": "string", "description": "Human-readable service name"},
            "team": {"type": "string", "description": "Owning engineering team"},
            "tier": {"type": "string", "description": "tier-1|tier-2|tier-3 (criticality)"},
            "data_classification": {"type": "string", "description": "public|internal|confidential|restricted"},
            "dependencies": {"type": "array", "description": "Downstream service IDs"},
            "pii_handler": {"type": "boolean", "description": "Whether service handles PII"},
            "external_facing": {"type": "boolean", "description": "Whether service is internet-accessible"},
            "language": {"type": "string", "description": "Primary programming language"},
            "deploy_target": {"type": "string", "description": "k8s|ecs|lambda|vm|on-prem"},
            "slo_availability": {"type": "number", "description": "Target availability SLO (0-1)"},
            "last_security_review": {"type": "datetime", "description": "Last security design review"},
            "event_type": {"type": "string", "description": "registered|updated|deprecated|decommissioned"},
            "changed_at": {"type": "datetime", "description": "Event timestamp"},
        },
        sample_events={
            "service_registered": [{
                "service_id": "payment-svc", "service_name": "Payment Service",
                "team": "payments-eng", "tier": "tier-1", "data_classification": "restricted",
                "dependencies": ["fraud-svc", "ledger-svc", "notification-svc"],
                "pii_handler": True, "external_facing": True, "language": "Go",
                "deploy_target": "k8s", "slo_availability": 0.9999,
                "last_security_review": "2026-03-15T00:00:00Z",
                "event_type": "registered", "changed_at": "2026-06-01T10:00:00Z",
            }],
            "service_deprecated": [{
                "service_id": "legacy-auth", "service_name": "Legacy Auth Service",
                "team": "identity-eng", "tier": "tier-2",
                "event_type": "deprecated", "changed_at": "2026-06-02T09:00:00Z",
            }],
        },
        mitre_mappings={
            "T1195.002": ["service_registered"],
            "T1078": ["service_deprecated"],
        },
        doc_url="",
        doc_last_checked="2026-06-02",
        update_frequency="stable",
        splunk_sourcetype="product_registry:service",
        joti_source_type="product_registry",
    ),
]
